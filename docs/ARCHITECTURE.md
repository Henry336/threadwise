...
Study Markdown notes remain ordinary `StudyResource` rows, so Telegram capture, module/session links, search, and analysis evidence do not fork into a browser-only document store. `StudyResourceRevision` retains at most 50 encrypted title/body snapshots per note; dashboard detail returns only the newest bounded window. `StudyNoteLink` stores resolved workspace-local edges for backlinks, while unresolved links remain visible in the reader and can resolve after the target note exists and the source is saved again. Dashboard updates include the previously observed `updatedAt` value and fail with a conflict before overwriting a newer note. Raw HTML and scripts never execute; Mermaid is rendered lazily by the dashboard under strict mode and sanitized again before insertion. Telegram derives a plain-text fallback and replaces diagrams with an explicit dashboard-viewing marker.
