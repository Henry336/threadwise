...
Study write routing is explicit. `activeModuleId` is navigation state, not a default destination:
captures without one unambiguous module reference or module-specific bot reply are persisted as
`StudyPendingCapture` rows until the owner chooses an active module. Canvas source metadata follows
