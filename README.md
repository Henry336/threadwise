...
- Gives each group a separate responsive dashboard with a concise overview, attention queue, assignee workload, filtered shared tasks, stand-up view, activity history, and a lightweight seven-day summary.
...
`/dashboard` inside a group opens that group's separate shared web workspace. The bot should be a group administrator before members use this link: Telegram only guarantees live `getChatMember` checks for other users when the bot is an administrator. If that verification is unavailable, Threadwise fails closed rather than exposing shared content. Group settings require an owner or administrator; shared captures and collection actions are available to verified active members. Gmail, Calendar, Excel, personal export, and account deletion remain personal-only.

The shared dashboard is deliberately practical rather than managerial theatre: **Group overview** surfaces overdue, unassigned, awaiting-reply, and blocked work; **People** shows assignment load without ranking people; **Stand-up** derives done, next, and blocked items; and **Activity** records meaningful task movement. Task changes made on the web use the same database rows queried by the bot and are quietly mirrored into the Telegram group.
